...
# Track question index in session
if "question_index" not in st.session_state:
    st.session_state.question_index = 0
...
if question_index < total_questions:
    ...
    # Format as "1 of 10", etc.
    progress_text = f"Ibéèrè {question_index + 1} nínú {total_questions} ({yoruba_num})"
    st.markdown(f"### {progress_text}")
    st.write(selected_question["question"])

    # Construct TTS script
    question_text = (
        f"{progress_text}. {selected_question['question']}... "
        f"A... , {selected_question['option_a']}... "
        f"B... , {selected_question['option_b']}... "
        f"C... , {selected_question['option_c']}... "
        f"D... , {selected_question['option_d']}."
